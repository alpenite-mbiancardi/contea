...
<c:if test="${not empty param['src_properties(jnt:news).type.value']}">
	<c:set var="type" value="${param['src_properties(jnt:news).type.value']}" />  
</c:if>
<c:choose>
...
<c:url var="url" value='${url.base}${currentNode.path}.xml?servizio=${type}&lang=${language}&num=10'/>
