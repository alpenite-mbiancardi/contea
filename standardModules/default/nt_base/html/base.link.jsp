...
<c:if test="${not empty param.linkOptions}"><c:set var="linkOptions">${param.linkOptions}</c:set></c:if>
...
<c:if test="${not omitFormatting}"><span class="icon ${cssClass}"></span></c:if><a href="${urlValue}"${linkTitle} ${linkOptions}>${fn:escapeXml(not param.useNodeNameAsTitle && not empty title.string ? title.string : currentNode.name)}</a>
